...
#
# The dirty check deliberately excludes common build-artifact directories
# (target/, build/, .gradle/, node_modules/, dist/, out/). Those directories
# are often regenerated by a build/test run even when no source files changed,
# so their presence would otherwise produce false-positive recovery triggers.
SKIP_AIDER=false
_DIRTY_EXCLUDES=(':!target' ':!build' ':!.gradle' ':!node_modules' ':!dist' ':!out')
if ! git diff --quiet -- "${_DIRTY_EXCLUDES[@]}" || \
   ! git diff --cached --quiet -- "${_DIRTY_EXCLUDES[@]}"; then
    echo "==> Uncommitted source changes detected — running pre-flight verify (interrupted-run recovery check)..."
    PREFLIGHT_EXIT=0
    set +e
