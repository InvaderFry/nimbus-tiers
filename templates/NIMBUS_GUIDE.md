...
4. Calls Aider; `map-tokens: 0` and `edit-format: diff` are applied automatically from `.aider.conf.yml` to stay within the 10K token context window. **Exception:** when every file listed in `## Files to change` is brand-new (none exist on disk yet), `phase2.sh` overrides the edit format to `--edit-format whole` for that step. Whole-file format is more reliable for local models writing new files from scratch — it avoids the SEARCH/REPLACE empty-block protocol that small models get wrong. For steps that mix new and existing files the `diff` default is kept. Aider runs under a 15-minute wall-clock timeout (bounded blast radius if the model gets stuck). Note: `--max-reflections` is not passed — it is not exposed by the installed Aider version.
...
| `.aider.conf.yml` | Aider config (Path C defaults: local TabbyAPI, `map-tokens: 0`, `edit-format: diff`). `phase2.sh` overrides to `--edit-format whole` for purely greenfield steps (all planned files are new). |
